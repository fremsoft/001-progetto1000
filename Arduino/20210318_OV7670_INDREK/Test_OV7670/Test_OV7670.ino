...
void processLine( uint8_t n_linea ) {
  ...
  for (j = 0; j < 320; j+=(2*SCALE_FACTOR)) {
    ...
    Serial.print( "<td title=\""); 
    Serial.print( j /* x */ ); Serial.print( ","); Serial.print( n_linea /* y */ );
    Serial.print( "\" style=\"width:4px; height:4px; font-size:1px; background: #");
...
void loop() {
  ...
  while (true); /* ciclo infinito: ferma esecuzione */
