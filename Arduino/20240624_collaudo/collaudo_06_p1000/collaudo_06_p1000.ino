/*
  ...
  * gestione dei sensori degli encoder a interrupt
  ...
  https://www.youtube.com/live/M33s9mqJizs
  
  Bologna, 26 agosto 2024
*/
...
sprintf(s, "DX=%d; SX=%d, r=%d, s=%d"
  ...
  switch (stato_macchina) {
    ...
    case ST_CERCA_APERTURA_MANO_DX:
      if (regime == 0) {
        if (deltaT_encoder_presa_mano_DX() < DELTA_T_BUCO_ENCODER) {
    ...
    case ST_CERCA_APERTURA_MANO_SX:
      if (regime == 0) {
        if (deltaT_encoder_presa_mano_SX() < DELTA_T_BUCO_ENCODER) {
          //ogni volta che c'è un cambio di stato nell'encoder
          regime = 1;
          stato = get_corsa_mano_SX();
        }
    ...
    case ST_MANO_SX_APERTA:
      ...
      setSpeedMotoreDxRotazione(  300 );
      setSpeedMotoreSxRotazione( -300 );
      while (getRotazionePolsoSX() != DITO_CENTRALE_ALTO) /* do nothing */;
      setSpeedMotoreSxRotazione( 0 );
      while (getRotazionePolsoDX() != DITO_CENTRALE_BASSO) /* do nothing */;
    ...
    case ST_APRI_CHIUDI:
      if (joystickDxPressed()) {
        ...
        if (get_corsa_mano_DX() <= 0) {
          ...
          setSpeedMotoreDxPresa(FORZA_MANTENIMENTO_CHIUSURA_MANO_DX);   
        }
        else {
      ...
      if (joystickSxPressed()) {
        ...
        if (get_corsa_mano_SX() <= 0) {
          ...
          // la mano ha afferrato qualcosa (o è arrivata al buco)
          setSpeedMotoreSxPresa(FORZA_MANTENIMENTO_CHIUSURA_MANO_SX);   
        }
        else {
          // la mano sta afferrrando il cubo
          // la apriamo per mollare il cubo
